...
```
<pre class="src src-sh">git add resources
git commit -m <span style="color: #8b2252;">'application name and client config'</span>
```
...
```
<pre class="src src-sh">=> ... details elided ...
...
```
<pre class="src src-sh">clojure -A:ion-dev <span style="color: #8b2252;">'{:op :deploy :rev $(</span><span style="color: #cd00cd;">REV</span><span style="color: #8b2252;">) :group $(</span><span style="color: #cd00cd;">GROUP</span><span style="color: #8b2252;">)}'</span>
```
...
```
<pre class="src src-sh">clojure -A:ion-dev <span style="color: #8b2252;">'{:op :deploy-status :execution-arn $(</span><span style="color: #cd00cd;">EXECUTION_ARN</span><span style="color: #8b2252;">)}'</span>
```
...
```
<pre class="src src-clojure">{<span style="color: #008b8b;">:deploy-status</span> <span style="color: #8b2252;">"SUCCEEDED"</span>, <span style="color: #008b8b;">:code-deploy-status</span> <span style="color: #8b2252;">"SUCCEEDED"</span>}
```
