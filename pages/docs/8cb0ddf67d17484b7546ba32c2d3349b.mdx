...
```
<pre class="src src-sh">datomic client access <system>
```
...
```
<pre class="src src-sh">curl -x socks5h://localhost:[port] http://entry.[system].[region].datomic.net:8182/
```
...
```
<pre class="src src-clojure">{<span style="color: #008b8b;">:s3-auth-path</span> [bucket-name]}
```
