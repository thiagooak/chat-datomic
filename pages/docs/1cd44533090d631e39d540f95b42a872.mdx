...
Datomic [datalog queries](supported-ops.html#datalog) automatically use multiple [indexes](architecture.html#indexes) to support a variety of access patterns. In addition to supporting query, these indexes support [identity and uniqueness](#identity-and-uniqueness), an indelible [time model](#time-model), and [lookup refs](#lookup-refs).
...
E42A:user/favorite-colorV:blueTx1234Optrue
...
EAVTxOp42:user/favorite-color:blue1234true42:user/first-name"John"1234true42:user/last-name"Doe"1234true42:user/favorite-color:green4567true42:user/favorite-color:blue4567false
...
EAVTxOp42:user/first-name"John"1234true42:user/last-name"Doe"1234true42:user/favorite-color:green4567true
...
EAV42:user/first-name"John"42:user/last-name"Doe"42:user/favorite-color:greenThis three-tuple view is very similar to a programming language object where the *E* is analogous to *this* or *self*. The *map view* of an entity at a particular point in time captures this information more compactly, using the reserved pseudo-attribute name `:db/id` for *E*:

```
<pre class="src src-clojure">{<span style="color: #008b8b;">:db/id</span> 42
 <span style="color: #008b8b;">:user/favorite-color</span> <span style="color: #008b8b;">:green</span>
 <span style="color: #008b8b;">:user/first-name</span> <span style="color: #8b2252;">"John"</span>
 <span style="color: #008b8b;">:user/last-name</span> <span style="color: #8b2252;">"Doe"</span>}
```
...
EAV7:inv/id"SKU-1234"7:inv/color:green8:inv/id"SKU-5678"8:inv/watts600008:doc/url"http:…"Notice that, other than `:inv/id`, entities 7 and 8 have entirely disjoint attributes.
...
For more information see the [schema documentation](../schema/schema-reference.html).
...
EAVTxOp42:election/winner"Dewey"1234true42:election/winner"Dewey"1235false42:election/winner"Truman"1235true1235:correction/for12341235trueDatomic's time model allows complete recovery of the data "as of" any point in time, and it also allows queries that look at differences between two or more points in time.

For more information, see the [Datomic Time Model Documentation](../time/log.html).
...
- When you create a datom, Datomic automatically assigns it a database-unique number called an [entity id](../schema/schema-reference.html#db-id).
- The built-in [:db/ident attribute](../schema/schema-reference.html#db-ident) can be used for programmatic names.
- The built-in [:db/unique attribute](../schema/schema-reference.html#db-unique) can be added at any time. The uniqueness properties of an existing attribute also can be changed at any time (so long as the uniqueness properties are valid for the data). The :db/unique attribute can be used in the following ways:
...
- Lookup refs cannot be used in the body of a query though they can be used as inputs to a [parameterized query](../query/query-executing.html#work-with-data-structures).
