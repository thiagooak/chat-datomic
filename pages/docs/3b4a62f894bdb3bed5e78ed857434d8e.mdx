...
```
<pre class="src src-example">. venv/bin/activate
```
...
```
<pre class="src src-example">superset run -p 8080 --with-threads --reload --debugger
```
...
```
<pre class="src src-example">presto://<host>:<port>/<catalog>/<schema>
```
