...
```
<pre class="src src-example">bin/rest -p port [-o origins]? [alias uri]+
```
...
```
<pre class="src src-example">datomic:ddb://aws-region/ddb-table?aws_access_key_id=XXX&aws_secret_key=YYY
```
...
```
<pre class="src src-example">dev datomic:dev://localhost:4334/
#+end_src example
...
```
<pre class="src src-example">curl http://localhost:8001/data/
...
```
<pre class="src src-example">curl http://localhost:8001/data/dev/
...
application/edn{:db-name "new-db-name"}application/x-www-form-urlencodeddb-name=new-db-name```
<pre class="src src-example">curl -X POST -H "Content-Type: application/edn" http://localhost:8001/data/dev/ -d "{:db-name scratch}"
...
application/edn{:tx-data \[{:db/id "foo" …}\]application/x-www-form-urlencodedtx-data=\[{:db/id "foo" …}\]```
<pre class="src src-example">curl -X POST -H "Content-Type: application/edn" http://localhost:8001/data/dev/scratch/ -d "{:tx-data [{:db/doc \"hello REST world\"}]}"
...
```
<pre class="src src-example">curl http://localhost:8001/data/dev/mbrainz-1968-1973/-/
...
```
<pre class="src src-example">curl http://localhost:8001/data/dev/mbrainz-1968-1973/-/datoms\?index\=avet\&e\=\&a\=\&v\=\&start\=\&end\=\&offset\=\&limit\=100\&as-
of\=\&since\=
...
```
<pre class="src src-example">curl http://localhost:8001/data/dev/mbrainz-1968-1973/-/entity\?e\=17592186048482\&as-of\=\&since\=
...
application/edn{:q \[:find …\] :args \[{:db/alias … }\]}application/x-www-form-urlencodedq=\[:find …\]&amp;args=\[{:db/alias …}\]```
<pre class="src src-example">curl "http://localhost:8001/api/query?q=%5B%3Afind+%3Fe+%3Ain+%24+%3Fname+%3Awhere+%5B%3Fe+%3Aartist%2Fname+%3Fname%5D%5D&args=%5B%7B%3Adb%2Falias+%22dev%2Fmbrainz-1968-1973%22%7D+%22The+Who%22%5D&offset=&limit="
...
```
<pre class="src src-example">http://localhost:8001/data/dev/mbrainz-1968-1973/-/events
```
...
```
<pre class="src src-example">curl -H "Accept: text/event-stream" http://localhost:8001/events/dev/mbrainz-1968-1973
```