...
The Datomic information model is a graph, but analytics tools invariably expect data to be in rectangles. While there are many ways to present data as rectangles (CSV files etc), all analytics tools support data from relational, SQL sources. We've developed a Datomic driver for [Presto SQL](https://trino.io/), a SQL query engine, and incorporated a Presto server into the access gateway for Datomic Cloud (see [Configuration](analytics-configuring.html)). Presto in turn supplies a [SQL command-line interface](analytics-cli.html), and there is native Presto protocol support for [Python](analytics-python.html), [R](analytics-R.html), [Metabase](analytics-metabase.html), [Superset](analytics-superset.html), [Tableau, and more](analytics-other.html). Presto also supports [JDBC](analytics-jdbc.html) for integration with Java-based analytics tooling. In short, if you are looking to connect an analytics tool to Datomic, you will look for the "presto" connector, or use JDBC.

[![analytics.svg](../images/analytics.svg)](../images/analytics.svg)
