...
cognitect.anomalies. See <a href="https://github.com/cognitect-labs/anomalies">https://github.com/cognitect-labs/anomalies</a>.
...
```
<pre class="plaintext">Returns datoms from an index as specified by arg-map:
...
```
<pre class="plaintext">Queries for database stats.
...
```
<pre class="plaintext">Walks an index, pulling entities via :e if :avet or :v if :aevt,
using the selector.
​
    :index     :avet or :aevt
...
```
<pre class="plaintext">Returns a hierarchical selection described by the keys in arg-map:
...
```
<pre class="plaintext">Performs the query described by arg-map's :query and :args:
...
```
<pre class="plaintext">Doing the work of qseq that is common to sync/async
```
...
```
<pre class="plaintext">Used to coordinate with other clients.
...
```
<pre class="plaintext">Retrieve transactions from the log as specified by arg-map:
...
```
<pre class="plaintext">Returns a channel of a with-db value suitable for passing to 'with'.
```