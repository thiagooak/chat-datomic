...
```
<pre class="src src-java">txResult = conn.transact(tx).get();

future = conn.transactAsync(tx);

... <span style="color: #b22222;">// </span><span style="color: #b22222;">do other work</span>
...
```
<pre class="src src-java">queue = conn.txReportQueue();

report = (<span style="color: #228b22;">Map</span>) queue.poll();
```
...
```
<pre class="src src-clojure">[<span style="color: #008b8b;">:find</span> ?e ?aname ?v ?added
 <span style="color: #008b8b;">:in</span> $ [[?e ?a ?v _ ?added]]
...
```
<pre class="src src-java">tx = Util.list(
       Util.map(<span style="color: #8b2252;">":db/id"</span>, <span style="color: #8b2252;">"datomic.tx"</span>,
                <span style="color: #8b2252;">":data/src"</span>, <span style="color: #8b2252;">"catalog-2_29_2012.xml"</span>),
       Util.map(<span style="color: #8b2252;">":product/name"</span>, <span style="color: #8b2252;">"Marbles"</span>));
