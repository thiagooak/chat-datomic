...
```
<pre class="example" id="orgd590cb1" linkedtable="true">pattern             = [attr-spec+]attr-spec           = attr-name | wildcard | map-spec | attr-exprattr-name           = an edn keyword that names an attrmap-spec            = { ((attr-name | attr-expr) (pattern | recursion-limit))+ }attr-expr           = [attr-name attr-option+] | legacy-attr-expras-expr             = [attr-name :as any-value]limit-expr          = [attr-name :limit (positive-number | nil)] default-expr        = [attr-name :default any-value]xform-expr          = [attr-name :xform symbol]attr-option         = as-expr | limit-expr | default-expr | xform-exprwildcard            = "*" or '*'recursion-limit     = positive-number | '...'legacy-attr-expr    = legacy-limit-expr | legacy-default-exprlegacy-limit-expr   = [("limit" | 'limit') attr-name (positive-number | nil)]legacy-default-expr = [("default" | 'default') attr-name any-value]
```
